...
async def annonc(
    ...
    print(f"[bot4.py] Команда от {interaction.user}")
    ...
    channel = client.get_channel(CHANNELS["bot4"])
    if not channel:
...
@client.event
async def on_ready():
    print(f"[bot4.py] Запущен как {client.user}")
    try:
        synced = await tree.sync()
        print(f"[bot4.py] Синхронизированы команды: {[cmd.name for cmd in synced]}")
    except Exception as e:
        print(f"[bot4.py] ❌ Ошибка при sync: {e}")

def run():
    client.run(TOKENS["bot4"])
