...
async def annonc(
    ...
    print(f"[bot1.py] Команда от {interaction.user}")
    ...
    channel = client.get_channel(CHANNELS["bot1"])
    if not channel:
...
@client.event
async def on_ready():
    print(f"[bot1.py] Запущен как {client.user}")
    try:
        synced = await tree.sync()
        print(f"[bot1.py] Синхронизированы команды: {[cmd.name for cmd in synced]}")
    except Exception as e:
        print(f"[bot1.py] ❌ Ошибка при sync: {e}")

def run():
    client.run(TOKENS["bot1"])
